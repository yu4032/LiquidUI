...
/**
 * Window ViewRoot lifecycle signal only. Vendor PassBlur transactions are deliberately executed
 * after these callbacks return; the observer owns no producer or rendering state.
 */
final class ViewRootSurfaceObserver {
    interface Listener {
        void onSurfaceDestroyed();
        void onSurfaceReady(String event);
    }

    private static final String TAG = "[GlassCore][RootSurface]";
    ...
    ViewRootSurfaceObserver(View host, Listener listener) {
        this.host = host;
            ...
            Object nextCallback = Proxy.newProxyInstance(
                    ...
                    (proxy, method, args) -> {
                        ...
                        if ("surfaceDestroyed".equals(name)) {
                            // Presentation revocation must be immediate; no stale frame may remain
                            // authoritative after its root endpoint has disappeared.
                            listener.onSurfaceDestroyed();
                        } else if ("surfaceCreated".equals(name)
                                || "surfaceReplaced".equals(name)) {
                            // Producer/rebind work must happen after ViewRoot's own transaction.
