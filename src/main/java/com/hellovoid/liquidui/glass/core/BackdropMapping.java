...
/** Pure geometry for mapping a Window-local glass sample rectangle into the PassBlur root frame. */
final class BackdropMapping {
    enum Coverage {
        FULL,
        PARTIAL,
        OUTSIDE
    ...
    static Result compute(
        ...
        // Android screen coordinates are top-left based while shader UVs are bottom-left based.
        float validBottom = clamp01(
                1f - (intersectionBottom - hostTop) / (float) hostHeight);
        float validTop = clamp01(
                1f - (intersectionTop - hostTop) / (float) hostHeight);
