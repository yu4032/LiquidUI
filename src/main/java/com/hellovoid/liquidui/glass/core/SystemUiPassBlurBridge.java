...
/** Window-scoped bridge to HyperOS's native PassBlur producer authority. */
final class SystemUiPassBlurBridge {
    private static final String TAG = "[GlassCore][PBGL]";
    // Native-resolution source pixels are required by the shared Prismal refraction path.
    // Keep this at 1.0: producer geometry and SurfaceTexture transform remain authoritative.
    ...
    /** Resolve the current ViewRoot SurfaceControl so callers never retain stale root state. */
    static Binding bind(View materialHost, Surface producerSurface, long endpointGeneration) {
    ...
    static Binding bind(
        ...
        try {
            ...
            if (!(rootValue instanceof SurfaceControl hostRootSurface)
                ...
                log("bind unavailable host root changed");
                return null;
            }
            ...
            // Do not inject the vendor transaction into ViewRootImpl's own surface replacement
            // transaction. Use a fresh caller-owned transaction after the lifecycle callback.
            ...
            log("bound root=" + binding.hostRootName
                    ...
                    + " scale=" + SCALE
                    + " sourceAuthority=ViewRoot-native");
            return binding;
        } catch (Throwable error) {
    ...
    /** Mark a root-owned endpoint stale without touching its long-lived producer BufferQueue. */
            ...
            if (binding.hostRootSurface.isValid()) {
                try (SurfaceControl.Transaction transaction = new SurfaceControl.Transaction()) {
                    binding.setPassBlurSurface.invoke(
                            transaction, binding.hostRootSurface, (Object) null);
                    binding.setUpdateTextureFlag.invoke(
