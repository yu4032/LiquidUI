...
| `com.android.systemui.statusbar.notification.row.ExpandableNotificationRowInjector` | `classes2.dex` | `updateBackground$1()` routes standard rows through `updateBlurBg(int,int,boolean)`; no-blur branch disables view blur and clears blend colors | Exact blur-authority boolean argument hook |
| `com.android.systemui.statusbar.notification.utils.NotificationUtil` | `classes2.dex` | `applyElementViewBlend(...)` enables `MiViewBlurMode=1` and installs MIUI background blend colors on the notification background view | Confirms native blur/blend layer that overrode first red build |
