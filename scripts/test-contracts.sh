        ...
        for (String className : args) {
            ...
            for (Method method : type.getDeclaredMethods()) {
                ...
                try {
                    ...
                    System.err.println("FAIL " + className + "#" + method.getName() + ": " + cause);
...
mapfile -t DIAGNOSTICS_MAIN < <(
    find "$ROOT/src/main/java/com/hellovoid/liquidui/diagnostics" -name '*.java' -print | sort
)
PURE_MAIN+=("${DIAGNOSTICS_MAIN[@]}")
mapfile -t TESTS < <(find "$ROOT/src/test/java" -name '*.java' -print | sort)

javac --release 17 -d "$WORK/classes" \
    "$WORK/stubs/org/junit/Test.java" \
    "$WORK/stubs/org/junit/Assert.java" \
    "$WORK/TestRunner.java" \
    "${PURE_MAIN[@]}" \
    "${TESTS[@]}"

mapfile -t TEST_CLASSES < <(
    find "$ROOT/src/test/java" -name '*Test.java' -print | sort | \
    sed -e "s#^$ROOT/src/test/java/##" -e 's#/#.#g' -e 's#\.java$##'
)

(
    cd "$ROOT"
    java -cp "$WORK/classes" TestRunner "${TEST_CLASSES[@]}"
)

# Text-level project invariants that do not need the Android SDK.
test "$(cat "$ROOT/src/main/resources/META-INF/xposed/scope.list")" = "com.android.systemui"
grep -qx 'com.hellovoid.liquidui.ModuleMain' \
    "$ROOT/src/main/resources/META-INF/xposed/java_init.list"
! grep -R -nE 'com\.miui\.home|com\.hellovoid\.liquiddock|Prismal' \
    "$ROOT/src/main" "$ROOT/build.gradle.kts" "$ROOT/settings.gradle.kts"
